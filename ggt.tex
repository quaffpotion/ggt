...
\begin{lem}
    ...
    Without loss of generality let \(z\) in \(T_3\). Consider the slice \(S = \text{cell}(A) \cap T_1 \times T_2 \times \{z\}\). We need to show that \(S\) is connected. Let \(p, q \in S\). Using the cell structure we can find a point \(p'\in A\) such that \(p\) is contained in the closure of a cell containing \(p'\). For a product of trees, this means we can find a closed \(n\)-cube called \(P\) containing \(p\) and \(p'\). Similarly for \(q\), \(q'\) and \(Q\).
    
    
    Consider the projection of \(P\) and \(Q\) to \(T_3\). Closed \(n\)-cubes project to edges or vertices in \(T_3\). Now, \(p,q\) have the same \(T_3\) coordinate; so \(z\) is contained in \(\pi_3(P) \cap \pi_3(Q)\). Therefore, the union \(\pi_3(P) \cup \pi_3(Q)\) is connected and \(d_{T_3}(z,p'_3), d_{T_3}(z,q'_3) \leq 1\).
    ...
    If \(p'_3 \neq  q'_3\) then without loss of generality we have, 
