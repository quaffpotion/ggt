...
\begin{defn}
    [Subgraph coning]
    \label{defn:coneoff}
    Consider an equivariant map $f: X \to T$ from a 2-complex $X$ to a tree $T$. Let $\Lambda(X,K,f)$ (or just $\Lambda(X)$) denote the space obtained by gluging $G$ translates of $(K \times I) / R$ to $X$ where $R$ is an equivalence relation that quotients one end of the product to $f(K)$.
\end{defn}
\begin{defn}
    [Minimal Action]
    \label{defn:minimal}
...
\begin{thm}
    ...
    \begin{proof}
        ...
        Consider \(J := \text{Im}(f)\). We need the action of $G$ on $J$ to be free and properly discontinuous with a compact quotient. We also need $J$ to have Property CCP. After establishing this we will add material to $J$ and prove that these properties are preserved at each step in order to obtain a sort of extended core denoted by $E$.
        
        We use the fact that the tree actions are simplicial and so the diagonal action is cellular. This with freeness from being pairwise transverse implies the action is PDC, then \ref{lem:pdsubspace} gives PDC for subsets with the subset topology (In particular, we should not have to demand that $J$ be a subcomplex off the bat)
        
        By lemma \ref{lem:fibershomeoplanes} we get that \(J\) has property CCP \ref{defn:ccp}, since \(J\) is the image of a cocompact set under a continuous \(G\)-map it is cocompact. Let \(S_{12}\) denote the cellular neighborhood of \(J\), because this set is defined in an invariant way as the smallest such complex with a property it is \(G\) invariant and because our trees are locally finite and a cellular neighborhood is contained in a bounded neighborhood we have that \(S_{12}\) is cocompact. Taking a cellular neighborhood respects slices by \ref{lem:cellrespecslice}  so \(S_{12}\) has property CCP \ref{defn:ccp} as well. Technically, we apply \ref{lem:pdsubspace} again to get properly discontinuous.

        Next we repeatedly apply lemma \ref{lem:fillingcocompact} and use the fact that filling preserves CCP \ref{defn:ccp} by construction to get that \(E:= S_{12}xyz\) is cocomapct and has CCP \ref{defn:ccp}. By switching \ref{lem:switching} \(C\) has connected 1-dimensional fibers. 
...
\begin{lem}
 ...
 Let $G$ a group of dimension 2. If $X$ and $Y$ are non-trivial $G$-trees of finite type that are in different deformation spaces then the following are equivalent:
\begin{enumerate}
    \item $X$, $Y$ transverse
...
\begin{enumerate}
    ...
    \item $2\Rightarrow 3$: Take $X \times Y$ and give it the VH-structure where $X$ and $Y$ correspond to horizontal and vertical edges respectively. Apply the main theorem \ref{thm:guirardelcore} of a paper by Guirardel to obtain $C$ the core. By \ref{lem:nonempty} the core is non-empty. Condition (2) says that $G$ acts freely on $X\times Y$. By \ref{lem:coreconnected} the core is connected. By the remark after Proposition 4.17 in Guirardel the core is simply connected. The action is also free on subsets of $X \times Y$. Since the action is cellular and free it's a covering space action. (We avoid situations like irrational rotations on a circle that are free but not covering space actions) 
    
    We also need that $C/G$ is VH. Is it enough to say that the action respects the tree factors. (The edge partition on the cover descends to a well-defined edge partition on the quotient and attaching maps constructed in the standard way for the quotient alternate between vertical and horizontal edges)
    
    Lastly, the space $X \times Y$ is the product of two CAT(0) spaces and is therefore CAT(0). For 2-complexes, the NPC condition descends to subcomplexes. Hence, $C$ is NPC and so is $C/G$ since being NPC is a local condition and we have a covering map. This allows us to apply a result of Wise \ref{thm:wisegraph} to get the two splittings.
...
\begin{lem}
    ...
    \label{lem:shortcircuit} Suppose the hypotheses of \ref{lem:guirardel-extension}. In addition, suppose that $G$ is finitely presented. Then in the notation of Guirardel $\widehat X = (X_K, \mathscr{F}_K)$ i.e. when constructing $\widehat X$ we can stop after gluing certain translates to $X$.
    \begin{proof}
...
\begin{defn}
    ...
    \label{defn:ccp} 
    % TODO reword so that it's specialized to our case of just having 3 dimensions and ve very clear about fixing a factor
