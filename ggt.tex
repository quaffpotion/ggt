...
\begin{thm}
    If \(G\) has cohomological dimension 2 \ref{defn:cd}  then there are at most two deformation spaces of non-trivial finite type \ref{defn:finitetype} pairwise transverse \ref{defn:transverse} \(G\)-trees.
    \begin{proof}
        Suppose for sake of a contradiction there were three non-trivial \(G\)-trees \(T_{1}\), \(T_{2}\), and \(T_{3}\) of finite type \ref{defn:finitetype} that were pairwise transverse \ref{defn:transverse} and no two are in the same deformation space. Without loss of generality, we may assume that these are minimal \(G\)-trees since the theorem statement is true for general \(G\)-trees if it is true for minimal \(G\)-trees. \ref{lem:wlogmin}  Applying the tranverse construction lemma \ref{pro:transverseconstruction} we obtain \(X_{12}\). Let \(\widetilde {X_{12}}\) denote it's universal cover. Applying the affine equivariant map construction \ref{lem:affineequivariantmap} to the action on \(\widetilde {X_{12}}\) and \(T_{3}\) gives a map and a fibration that we denote by \(f_{123}\) with \(\mathscr{F}_{123}\) respectively.  Due to \ref{lem:simpgeo} we have geomeric actions. Then using \ref{lem:guirardel-extension} and the proof theoreof we obtain \({\widetilde {X_{12}}}^{+} \), an extension  \(f_{123}^{+}\), and a fibration \(\mathscr{F}_{123}\).  Next let  \(f_{121}\) and  \(f_{122}\)  denote the projection  maps from \(\widetilde {X_{12}}\) to \(T_{1}\) and \(T_{2}\) respectively. (aka Bass-Serre maps) Using \ref{lem:coneextension} we get extensions \(f_{121}^{\wedge}\) and \(f_{122}^{\wedge}\). Lastly we form the product map \(f:= f_{121}^{\wedge} \times f_{122}^{\wedge} \times f_{123}^{+}\). 

        Consider \(J := \text{Im}(f)\). By lemma \ref{lem:fibershomeoplanes} we get that \(J\) has property CCP \ref{defn:ccp}, since it is the image of a cocompact set under a continuous \(G\)-map it is cocompact and in fact is cocompact by factor \ref{defn:cocompactcrossfactor}. Let \(S_{12}\) denote the cellular neighborhood of \(J\), because this set is defined in an invariant way as the smallest such complex with a property it is \(G\) invariant and because our trees are locally finite and a cellular neighborhood is contained in a bounded neighborhood we have that \(S_{12}\) is cocompact and therefore cocompact by factor \ref{defn:cocompactcrossfactor}. Because filling respects cells \ref{pro:fillingrespectsslices} we get that \(S_{12}\) has property CCP \ref{defn:ccp}. 
        ...
        We need to verify that \(C\) is simply connected. To do this we will show that it is a graph of spaces where the edge and vertex spaces are simply connected and the underlying graph is a tree. Take the hyperplanes of \(C\) that are transverse to the vertical edges as the edge spaces and the connected components of the pieces remaining after removing the corridors of the hyperplanes as the vertex spaces. Because \(C\) has connected 1-dimensional fibers, we get that the hyperplanes of \(C\) also do so that we can apply Guirardel to get that the hyperplanes are quadrant convex and therefore simply connected. 

        Hyperplanes are defined via extending along parallelism classes. 

        Why vertex spaces simply connected?

        The maps from edge spaces to vertex spaces are defined by the push map in \(C\) from the hyperplane to the two sides. 


        Hence, it's slices are quadrant convex, and therefore simply connected. From this we get that \(C\) is a valid graph of spaces decomposition. Note that the edge spaces are simply connected and the underlying graph is a tree, hence by Scott and Wall \(C\) is simply connected. From the transverse \(G\)-trees we get that the action is free on pairs of trees and therefore \(G\) acts properly discontinuously on \(C\). Note that \(C\) is a VHD complex \ref{defn:vhd} and suppose the iterated splittings do not end in all trees. 

        We have now verified that \(C\) is a graph of spaces in three different ways. We will now iterate the splitting process and then apply Bieri. To begin, by assumption each splitting is non-trivial. Now, consider the vertex groups of a given splitting. By lemma \ref{lem:iteratedsplitting} those vertex groups act non-trivially on the two trees coming from splittings of the VH-complexes by Wise. These last splittings are themselves splittings over trivial groups - but are they trivial splittings? Here we need to assume that one such iterated sequence of splittings ends in a graph with positive rank.
...
\begin{defn}
    [VHD complex]
    \label{defn:vhd} 
    A complex \(X\) is VHD if it's links are simplicial graphs and the edges of \(X\) can be partitioned into three sets such that 
...
\begin{defn}
	[bad edge snipping]
    Let \(K\) satisfy property setup \ref{defn:setup}  with \(e\) a bad edge. Let \(\sigma_{i}\) and \(\sigma_{j}\) be the first and second subpaths of \(\sigma\) that use \(e\). Take \(\sigma '\) to be the concatenation of \(\sigma_{k}\) for \(k < i\) with the subpath denoted \(\sigma '_{ij}\) that is the concatentation of the largest initial subpath of \(\sigma_{i}\) not using \(e\) and the longest tail of \(\sigma_j\) not using \(e\) with \(\sigma_{k}\) for \(k > j\). The unmodified subpaths of \(\sigma\) in \(\sigma '\) receive the same colors as before and we take \(\sigma '_{ij}\) to have the color that \(\sigma_{i}\) and \(\sigma_{j}\) shared.
\end{defn}
...
\end{lem}
\begin{claim*}
    If \(K\) satisfies property setup \ref{defn:setup} then the first and last edge of \(\sigma\) are equal.
\end{claim*}
\begin{defn}
	[Filling]
...
To achieve this, we'll build \(C\) from a suitable \(G\)-invariant subcomplex \(K \subseteq \mathscr{T} \) by taking \(C = \sxyz\).  
...
\begin{lem}
    ...
    \begin{proof}
        We observe that \(C\) is a graph of spaces, in particular it is a tree of simply connected spaces. Hence by scott and wall it's simply connected
    \end{proof}
\end{lem}
...
\begin{lem}
 ...
 \label{lem:preservesetup} 
    If \(K\) satisfies property setup \ref{defn:setup} then \(K'\) obtained from snipping a bad edge also satisfies property setup.
    \begin{proof}
