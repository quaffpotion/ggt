...
\begin{lem}
    ...
    \begin{proof}
        Proprty CCP implies that \(S \cap (T_{1} \times \{r_{2}\} \times \overline\delta)\) is connected. Let \(\sigma\) be a path in that set from \(p\) to \(q\). Project and take a preimage to get \((( \pi_{3} \sigma )^{-1} (\{r_{3}\}))\)  a closed set in \([0,1]\). The complement is a countable disjoint union of open intervals. 
    \end{proof}
