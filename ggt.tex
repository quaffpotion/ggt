...
\begin{prop}
    ...
    \begin{proof}
        Guirardels' proof of prop 4.17 from \cite{guirardelcorepaper}.
    \end{proof}
\end{prop}
...
We will now iterate the splitting process and then apply Bieri. To begin, by assumption each of the original splittings \(T_1\), \(T_2\), and \(T_3\) are non-trivial. Now, consider the vertex groups of a given splitting. By lemma \ref{lem:iteratedsplitting} each of those vertex groups acts non-trivially on the other two trees. These non-trivial actions are the second iteration of the splitting process. Each vertex group from the original splitting is a fundamental group of a VH-complex, by \ref{thm:wisegraph} they split as a graph of spaces where the vertex and edge spaces are themselves graphs. The graphs can be regarded as splittings over the trivial group -- but are they trivial splittings? It could happen that every graph has trivial fundamental group. Here we need to assume that at least one sequence of iterated splittings ends in a graph with positive rank.
