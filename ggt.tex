...
\begin{lem}
    [Slice Switching]
    If \(R \subseteq T_{1} \times T_{2}\) is a connected \fix{(try to remove this assumption) subcomplex} then \( \left( R_{x} \right)_{y} = \left( R_{y} \right)_{x}\) where $x$ corresponds to $T_1$ and $y$ corresponds to $T_2$ in the definition of filling \ref{defn:filling}.
    \begin{proof}
