...
[Technical assumptions for Guirardel]
 ...
 We check the following properties for the map $f_{123}: \widetilde{X_{12}}\to T_3$ from \ref{lem:affineequivariantmap}. (Compare to \ref{lem:guirardel-extension})
    \begin{enumerate}
...
\begin{lem}
    ...
    Pick p, q in cell(A) intersect a slice.
    
    For a product of trees, the closure of a cell is a subcomplex.
    
    For all p in cell(A) there exists a cell P and a point p' in A such that p' in P and p in the topological closure of P.
    
    Similarly for q, q', and Q.
    
    Since p and p' are contained in a closed n-cube but p in P, we have that the Lsup distance between p and p' is less than 1
    
    Note, closed cubes project to closed cubes when projected to T3, and since p3=q3 (= z)
    the projections of cl(P) and cl(Q) are closed cubes in T3 both containing z.
...
Using the CCP property of \(A\), for a given point \(r\in T_3\) we can note the first and last time a path enters the slice \(T_1\times T_2\times \{r\}\) and replace that (possibly degenerate) segment by a path contained in \(A\cap T_1\times T_2\times \{r\}\).

Take the convex hull in the tree. If z is an extreme point then we can draw a path from p’ to q’ and only have to snip twice. If z is not an extreme point then it is in the middle and we will have to snip three times as vertices in T3 allow a path to wander arbitrarily far in the product metric. 

Once complete, the new path goes from p' to q', is contained in \(A\), and projects to within distance 1 of \(z\). If necessary, use the open cells containing p' and q', to draw paths to create a new path from p to q. Denote this path by \(\eta\) and form \(t\to (\eta_1(t), \eta_2(t), z)\) a path from p to q contained in \(\text{cell}(A)\) as \(\eta\) was contained in \(A\) and remained close to \(z\).
