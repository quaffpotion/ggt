...
\begin{lem}
    ...
    Suppose \(X \to Y\) is a collapse map bewteen \(G\)-trees of finite-type with finitely generated edge and vertex groups.  Suppose also that the image of the map contains more than one vertex. Then, if an element if hyperbolic for \(X\) it is also hyperbolic for \(Y\).
\end{lem}
