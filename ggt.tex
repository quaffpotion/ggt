...
\label{thm:wisegraph}[Wise, Thm 2.16 from Wise complete square complexes]
...
\begin{lem}
    ...
    \begin{proof}
        ...
        Part 2: Fibration details (??? details we almost get for free)
        Part 3: isometry on edges transverse (??? details we almost get for free)
    \end{proof}
\end{lem}
...
\end{prop}
\begin{proof}
    For (1)$\Rightarrow$(2) we will construct a $G$-map 
...
\begin{dfn}
    ...
    \label{defn:ccp} 
    (??? reword so that it's specialized to our case of just having 3 dimensions and ve very clear about fixing a factor)
...
We write \(A\) to mean an arbitrary ring with a unit. Take \(R\) to be a non-zero commutative ring with a unit. A resolution of a module \(M\) is a sequence of modules denoted \((P_k)_{0\leq k\leq \infty}\) where \[\cdots \to P_0\to M\] is exact. We say a resolution \[\cdots 0\to P_n\to \cdots\to P_0 \to M\] where \(n\) is the greatest integer such that \(P_n\) is a non-zero module has length \(n\).
...
\begin{lem}
    ...
    Without loss of generality let \(z\) in \(T_3\). Consider the slice \(S = \text{cell}(A) \cap T_1 \times T_2 \times \{z\}\). We need to show that \(S\) is connected. Let \(p, q \in S\). Using the cell structure we can find a point \(p'\in A\) such that \(p\in\text{cell}(A)\) is contained in the cellular closure of a cell containing \(p'\). For a product of trees, this means we can find a closed \(n\)-cube called \(P\) containing \(p\) and \(p'\) where \(p'\) does not lie on the boundary of \(P\). Similarly for \(q\), \(q'\) and \(Q\).
    
    Consider the projection of \(P\) and \(Q\) to \(T_3\). Closed \(n\)-cubes project to edges or vertices in \(T_3\). Now, \(p,q\) have the same \(T_3\) coordinate; so \(z\) is contained in \(\pi_3(P) \cap \pi_3(Q)\). Therefore, the union \(\pi_3(P) \cup \pi_3(Q)\) is connected and \(d_{T_3}(z,p'_3), d_{T_3}(z,q'_3) < 1\) because \(p'\) does not lie on the boundary of \(P\).
    
     We have \(p', q'\in A\) with \(A\) connected. We need a path between \(p\) and \(q\) that lies in \(S\). Note, \(p_3=q_3=z\). Start with a path \(\gamma\) in \(A\) between \(p'\) and \(q'\). We will construct a path in \(A\) that lies in a narrow enough corridor to ensure \(\text{cell}(A)\) picks up enough cells so we can find a new path that lies in the slice.
    
    If \(p'_3 = q'_3\) then using the fact that \(A\) is CCP, draw a path between \(p'\) and \(q'\) that lies in \(A \cap T_1\times T_2\times \{p'_3\}\). Call this new path \(\eta\).
    
    If \(p'_3 \neq  q'_3\) without loss of generality we have, 
    \[0 = t'_0 < t'_1 < t''_0 < t''_1 = 1\]
    where \(t'_0, t'_1\) are the first and last times \(\gamma\) enters and leaves the closed set \(T_1\times T_3\times \{p'_3\}\) and similarly for \(t''_0, t''_1\) and \(q'_3\).
    
    To see this, start with \(\gamma\) and modify it. If \(t'_0 = t'_1\) then \(\gamma\) intersects the slice at \(p'_3\) only at \(p'\). Define a new path by concatenating  the  constant path at \(p'\) with \(\gamma\). With this new path, check if \(t''_0 = t''_1\) and concatenate with the constant path at \(q'\) if necessary. These steps ensure \(t'_0 < t'_1\) and \(t''_0 < t''_1\). Since \(p'_3 \neq q'_3\) and \(\gamma\) starts in the slice at \(p'_3\) and ends in the slice at \(q'_3\) we know \(t'_1 < t''_0\). Denote this new path from \(p'\) to \(q'\) by \(\eta'\).
    
    Now while in this case, replace \(\eta'_{[t'_0,t'_1]}\) with a path that lies in \(A \cap T_1\times T_2 \times \{p'_3\}\) between \(\eta'(t'_0)\) and \(\eta'(t'_1)\). Similarly for \(\eta'_{[t''_0, t''_1]}\). Call this path \(\eta\); it's a path from \(p'\) to \(q'\) whose projection is contained in \(\pi_3(P)\cup \pi_3(Q)\). 
    
    Next, draw a path from \(p\) to \(p'\) that's contained in \(P\) and similarly for \(q\), \(q'\), and \(Q\). Attach these to \(\eta\) to obtain a path from \(p\) to \(q\). Finally, consider the path \(t\to (\eta_1(t), \eta_2(t), z)\). Because the \(T_3\) coordinates of \(P\), \(Q\), and \(\eta\) were contained in a small interval this new path is contained in \(\text{cell}(A)\) as needed.
