...
\begin{defn}
    [Bad]
    An edge \(e\) in a tree \(T\) is {\em bad} with respect to \((\sigma = \sigma_{1} \cdots \sigma_{N} , \lambda , A, x_{0} )\) where \(A \subseteq V(T)\) and \(x_{0} \in T\) if \(e\) separates some \(a \in A\) from \(x_{0}\) and \(\lambda\) is constant on the set of subpaths that use \(e\).
\end{defn}
\begin{defn}
	[complexity]
