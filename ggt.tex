...
\begin{pro}
    ...
    (Exercise p.93) in \cite{bieribook}
    For a cocompact locally finite \(G\)-tree with \(FP_\infty\) vertex and edge groups we have that the dimension of \(G\) is exactly one more than the dimension of a vertex or edge group.
\end{pro}
\begin{defn}
    [Bass-Serre Map]
    \label{defn:bsmap}
...
\begin{lem}
    ...
    \begin{proof}
        \fix{ Add a bit about ... g fixes e1 in X then since it's not collapsed g fixes phi(e1) in K, but phi(e1) has something e2 above it in Y, but this is injective and so again the element g is in the stab of that edge e2 in Y. Ten apply local finiteness } If our actions were both trival then they would be in the same deformation space, so the actions are non-trivial and each quotient graph contains an edge. Suppose there was a common refinement. This would mean there is an edge in the collapsed tree that has an edge above it in both trees. Let \(K\) be the stabilizer of this edge. It appears in all three trees. Since $X$ and $Y$ are locally finite, we have that the vertex groups of $X$ and $Y$ are commensurable. But the property of fixing a point is invariant under commensurability. Therefore, all vertex groups of the first tree are elliptic in the second tree and vice versa. Hence, both actions have the same elliptic subgroups which means they are in the same deformation space which contradicts our initial assumptions.
    \end{proof}
\end{lem}
