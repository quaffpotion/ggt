...
\begin{defn}
    ...
    \begin{enumerate}
        ...
        \item The subpaths \(\sigma_{i}\) are non-degenerate.
        \item \(\sigma_{1} (1) = \overline{ \sigma_{N} (-1) }\) 
        \item \(\lambda_{1} \neq \lambda_{N}\) 
...
\begin{defn}
    [Bad]
    An edge \(e\) in a tree \(T\) is {\em bad} with respect to \((\sigma = \sigma_{1} \cdots \sigma_{N} , \lambda)\) if \(e\) separates some \(a \in \text{endpoints} \smallsetminus \{\partial_{0} \sigma_{1} , \partial_{1} \sigma_{N}\}\) from \(\partial_{0}\sigma (1)\) and \(\lambda\) is constant on the set of subpaths that use \(e\).
\end{defn}
\begin{defn}
	[complexity]
