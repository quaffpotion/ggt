...
\begin{defn}
    [Bad]
    An edge \(e\) in a tree \(T\) is {\em bad} with respect to \((T, \sigma = \sigma_{1} \cdots \sigma_{N} , \lambda,x_{0} )\) if \(e\) separates some \(a \in \text{endpoints} \smallsetminus \{x_{0} \}\) from \(\partial_{0}\sigma (1)\) and \(\lambda\) is constant on the set of subpaths that use \(e\).
\end{defn}
\begin{defn}
	[complexity]
...
\begin{lem}
 ...
 \label{lem:preservesetup} 
    ...
    \begin{proof}
        Let \(e\) be a bad edge of \(K\), since \(\sigma_{1}\) and \(\sigma_{N}\) have different colors the edge \(e\) is in at most one of them. Furthermore, \(e \neq \sigma_{1} (1)\) because then a subpath other than the first and last would use \(x_{0}\) which isn't allowed by \ref{item:interior}. Hence, the snipped path \(\sigma '\) will begin with an initial segment of \(\sigma_{1}\) and end with a tail of \(\sigma_{N}\).This shows that properties \ref{item:loop} , \ref{item:nondegen} , \ref{item:sharededge} , \ref{item:colors} are preserved. Because \(\sigma '\) is a restriction, we also get proprties  \ref{item:interior}. 
    \end{proof}
\end{lem}
