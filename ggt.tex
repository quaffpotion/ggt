...
Take G acting on \(T_{3}\) a geometric action and note that G acting on \(\widetilde{K}\) is a free properly discontinuous action. From \ref{lem:equivariant} we can consturct a map \(f: \widetilde{K} \to T_{3}\) that is invariant. Let \(\mathscr{F}\) denote the set of connected components of fibers of \(f\). Using \ref{lem:guirardel-extension} we obtain \(\hat f : \mathscr{C} \to T_{3} \). 
\begin{lem}
    [Connected Fibers]
\end{lem}
\begin{lem}
