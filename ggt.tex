...
\begin{lem}
    ...
    Stopping after a certain point in Guirardel is beneficial.
\end{lem}
\begin{lem}
    [Cell Respects Slices]
    \label{lem:cellrespecslice} 
    by set theory we are taking cell of a 2D thing and so connected really
\end{lem}
\begin{lem}
    [Technical assumptions for Guirardel]
    \label{lem:measuredetc} 
    we use a result of leveitt and paulin for geomeric (see elsewhere) and prove the rest here
...
\begin{lem}
    ...
    \label{lem:coresc} 
    Our \(C\) inherits a VHD structure from it's product structure and because the action is diagonal we also get that the quotient \(C/G\) is VHD. Because the action on the upstairs trees does not invert edges we get that the hyperplanes of \(C/G\) are two-sided. (Recall, a given hyperplane only touches a single parallelism class of edges) Next, we need the push maps from the hyperplanes to be injective on fundamental groups. The push map from a hyperplane to a vertex space (we necessarily send hyperplanes - itself a connected set - to a component i.e. a vertex space) followed by inclusion is the same (up to homotopy) as globally including the hyperplane into \(C/G\). We will show that the composition is injective so that the induced map from the vertex to the edge is injective as needed. ( we need the technical part of the proof here) \fix{needs the complicated diagram part}
\end{lem}
