...
\begin{enumerate}
    ...
    \item Being directed is essentially a non-issue but Wise uses it along with NPC to ensure certain complexes are graphs of free groups
\end{enumerate}
\section{Summary}
...
\begin{thm}
    ...
    \begin{proof}
        ...
        Consider \(J := \text{Im}(f)\). We need $J$ to satisfy a few properties at the outset and they need to be preserved as we add material to $J$ to eventually obtain a kind of extended core $E$ which $G$ will act on.
        
        \fix{We start by proving that the action of $G$ on $J$ is PDC. [...]}
        We use the fact that the actions are simplicial and so the diagonal action is cellular. This with freeness from being pairwise transverse implies PDC for subsets with the subset topology (In particular, we should not have to demand that $J$ be a subcomplex off the bat)
