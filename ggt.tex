...
\label{lem:fillingcocompact}
    Since $S_{23}$ is a cocompact $G$-invariant subcomplex that's contained in \(A \times B\) where \(A\) and \(B\) are subcomplexes of \(T_1\) and \(T_2 \times T_3\) respectively and \(B\) is itself cocompact we have that $(S_{23})_x$ is cocompact as well.
\begin{proof}
Recall that $G$ acts freely on $T_2\times T_3$. For each vertex orbit of $B$ choose a particular vertex. Call them $\{b_1,\ldots,b_n\}$; by cocompactness this list is finite. By freeness in the second factor, the stabilizer of $S' := S_{23} \cap (T_1 \times \{b_1\})$ is trivial. This means that $S'$ injects into $S_{23}/G$ which is compact by assumption. Since we are dealing with cell complexes, $S'$ is also compact. Repeating this argument a finite number of times we have that $S_{23}\cap (T_1\times \{b_k\})$ is compact for each \(k\). Compact items have a well defined diameter, taking the maximum diamater and noting we're acting by isometries gives that there is a bound on the diameters of \(S_{23} \cap (T_1\times \{v\})\) as \(v\) ranges over vertices of \(B\). 



    \textcolor{blue}{Let \(a \times b\) be a vertex of \((S_{23})_x\). By construction, \(a \in \text{chull}(\text{projection}(\text{slice}(S_{23}, b)))\). 
    Hence there exists \(D\) such that slices of \((S_{ 23 })_x\) have diameter no greater than \(D\) in the product metric. 
    This means each slice of \((S_{23})_x\) has a finite number of vertices. Lastly, since \(S_{23}\) is $G$-invariant and we're acting by a product, (so the slice at \(gb\) is the slice at \(b\) acted on by \(g\)) which means there are a finite number of isometry classes of slices since \(B\) is cocompact. Hence, there are a finite number of vertex orbits so our set is cocompact as needed}
\end{proof}
\end{lem}

\subsection{Promote elliptic containment to equality}
\textcolor{blue}{
    Sketch: The first fact is if G has a hyperbolic element then H has a unique non-empty minimal tree that's the union of all the axes. From this we can prove that commensurable groups where one has a hyperbolic element gets you that both have the same non-empty minimal tree. From here, we think about what would happen if an element was hyperbolic got sent to an elliptic element. Say there are at least two points in Y. Looking at preimages, we get two stabilized trees. The stabilizer of one point in Y over in X stabilizes a tree and so the minimal subtree for that group is in that tree which is disjoint from the other stabilizer because of how functiotns work. Except by local finiteness the groups are the commensurable. So the trees are both equal and disjoint which is bad.
    - still need to say that inverse of a point is a tree
    - still need to say why stabs are commensurable
}
...
\begin{lem}
    ...
    \textcolor{blue}{[fix, correct as written] 
