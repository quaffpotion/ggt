...
\begin{defn}
    ...
    \begin{enumerate}
        \item \(T\) is a topological tree 
        \item \(T \smallsetminus x_{0}\) is connected, where \(x_{0}\) is a 0-cell.
...
\begin{defn}
    ...
    \begin{enumerate}
        \item \(e^{+} \cap \text{endpoints} \neq \varnothing\) where \(e^{+}\) is the halfspace of \(e\) not containing \(x_{0}\).
        \item \(\left| \{\lambda (k) \mid e \subseteq \text{Im} \sigma_{k}\} \right|=1\).
    \end{enumerate}
\end{defn}
\begin{defn}
	[path snipping]
    Let \(p\) be a loop in a tree. We say \(p'\) is the {\em snip} at an edge \(e\) of \(p\) if \(p'=p\mid_{[0,t_0]} p\mid_{[t_1,1]}\) where \(t_{0} = \sup \{t \mid p\mid_{[0,t)} \cap e  = \varnothing \}\) and \(t_{1} = \inf \{t > t_{0} \mid e \subseteq \text{Im} \sigma \mid_{ (t_{0} ,t] } \}\). When \(p=p_{1} \cdots p_{N}\) we take the subpaths of \(p'\) to be \(p'_{k} = p\mid_{[0,t_0]} p\mid_{[t_1,1]}\) when non-degenerate. 
\end{defn}
