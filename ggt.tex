...
\begin{thm}
    ...
    Main theorem, we can perhaps use this everywhere instead of using \ref{thm:forester}
\end{thm}
...
\begin{dfn}
    [Morphism]
    A \emph{morphism} is a map between graphs where vertices go to vertices and edges go to edges. (??? isn't this just a morphism in the category of graphs?)
\end{dfn}

\begin{dfn}
    [Collapse Map]
    A \emph{collapse map} is a map between two trees obtained by quotienting each connected component of a union of subtrees to a point.  (??? is this correct?)
\end{dfn}
...
\begin{dfn}
    [Hyperplane]
    (??? Cite the big special cube complex paper for this as it's very precise while being readable)
\end{dfn}
...
\begin{rmk}
Note that the link of every VH-complex is a bipartite graph; this follows from conditions (2) and (3), see also remark 1.4 from Wise \cite{wisethesis}. In dimension 2 the NPC property for a cube complex is equivalent to requiring that all links be simplicial graphs i.e. no double edges. Wise does not (??? in his decomp theorem he requires VH to be NPC) require his VH-complexes to be NPC. For our purposes we assume the equivalent link property in dimension 2 for convenience. The link property also generalizes to an object we need later called a VHD-complex.
\end{rmk}
...
\begin{dfn}
    [Refinement]
    (??? need to add) is this equivalent to the existence of a collapse map  that is compatible with the action?
\end{dfn}
...
\item $3\Rightarrow 2$: 
    ...
    By Wise, the action of $G$ on $\widetilde{K}$ extends to an action on $X \times Y$ with $\widetilde{K}\subset X\times Y$. By lemma \ref{lem:axeshausdorff} the axes are Hausdorff equivalent. Except, two unbounded sets from separate factors cannot be Hausdorff equivalent under the product metric, a contradiction.
...
\begin{thm}
    ...
    \begin{proof}
        Follows immediately from Lemma 5.4 and Corollary 5.5 from \cite{guirardelcorepaper}. Note that being quadrant convex implies connected fibers \cite{guirardelcorepaper}. (??? double check that this is correct)
    \end{proof}
\end{thm}
...
\end{lem}
        ...
        Next, define \(K := \text{cell}(J)\). By prop \ref{pro:cellinvariant}, \(K\) is \(G\) invariant and because our trees are locally finite and a cellular neighborhood is contained in a bounded neighborhood we have that \(K\) is cocompact. Taking a cellular neighborhood respects slices by \ref{lem:cellrespecslice}  so \(K\) has property CCP as well.
