...
\begin{thm}
    ...
    \begin{proof}
        \fix{this is now fixed due to \ref{lem:localfinhyper} and \ref{lem:commensurableminimal}.}
    \end{proof}
\end{thm}
...
\item $3\Rightarrow 2$: 
    ...
    Suppose $1\neq g \in G$ is an element of $G_x\cap G_y$ where $x \in V(X)$ and $y\in V(Y)$. Consider $\overline{x}$, the image of $x$ in $X/G$, a vertex of the quotient graph. Now, $X/G$ is the underlying graph for the horizontal splitting of $K$. Take a basepoint $b \in X_{\overline{x}}$, the vertex space in $K$ corresponding to $\overline{x}$. Since $g\in G_x$ we can represent it as a loop in $X_{\overline{x}}$ composed entirely of vertical edges. Lifting this to $\widetilde{K}$ gives a path composed of vertical edges. Repeatedly applying $g$ gives an a combinatorial axis. Do the same for $y$. Because $X\times Y$ is CAT(0) the combinatorial axes are hausdorff equivalent but because we're in a product metric and they exist in different factors this is impossobiel.
