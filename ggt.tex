...
\begin{thm}
    \label{mainthmf}
    [Forester, \cite{foresterdeformationrigidity}]
    (restate in the language of above, but add in the assumptions)
\end{thm}
...
Want to represent g as an honest loop in vertex
loops up to free homoto

===

1. g non-trivial in G_x
2. a conjugate g' of g can be represented as loop in X_xbar
3. the lift this loop is a path with different end points in tree Y bc covering space
4. covering space action, acting freely on a tree, so has an axis


===

unbased loop represents cc of an element 
...
===
    ...
    Suppose $1\neq g \in G$ is an element of $G_x\cap G_y$ where $x \in V(X)$ and $y\in V(Y)$. Consider $\overline{x}$, the image of $x$ in $X/G$, a vertex of the quotient graph. Now, $X/G$ is the underlying graph for the horizontal splitting of $K$. The complex $K$ is VH so $X_{\overline{x}}$ is a graph made of vertical edges. Since $g\in G_x$ we can represent it as a loop in $X_{\overline{x}}$. Lifting this to $\widetilde{K}$ gives a path in \(\{x\}\times Y\). Repeatedly applying $g$ gives a concatenation of paths which forms an axis composed of vertical edges in $\widetilde{K}$. Doing the same for $y$ gives an axis composed of horizontal edges. Call the vertical axis \(\ell_V\) and the horizontal axis \(\ell_H\).
    
    %(We could tighten to geodesics loops and lift or tighten afterwards)
    
    % (???
    %     One idea is to say the following:
    %     "G acts on X, Y. From K~ we get BS-maps to X, Y (Wise). The product of these BS-maps injects K~ into X x Y"
    %     ....
    %     .... K -> XxY (NOT onto)
    %     .... 
    %     .... Z on line, then product action on X x Y is diag translations ... K is a strip going diagonally
        
    %     However, BS-maps on definitionally surjective because they are quotients. Our K~ may not be a CSC so K~ may not necessarily be all of X x Y
    % )
    
    % ( 
    %     The action on K~ by convering translations is the restriction of the product action of G on X x Y -- using the BS product, we get K~ injected equivariantly into X x Y
        
    %     ...
        
    %     enough to locally inject bc NPC
        
        
    %     ...
        
    %     think about the annulus as a square ladder... trees are real lines ... action is Z
    % )
    
    
    
    % (Each splitting of K~ gives a BS-map, the product of these maps injects K~ into X x Y equivariantly. Therefore, the action on K~ by convering translations is the restriction of the product action of G on X x Y. )
    
    % ( All we need is the G-map K -> XxY from the product of the BS-maps )
    
    
    The map from \(\widetilde K\) to \(X \times Y\) given by the product of the Bass-Serre maps is a \(G\)-map that takes vertical edges to vertical edges and similarly for horizontal edges. Hence, the image of an axis in  \(\widetilde K\) is contained in a fiber of \(X \times Y\).
    
    Pick vertices \(v1, v2\in \widetilde K\) from the axes \(\ell_V\) and \(\ell_H\) respectively. Consider the images of \(v_1\) and \(v_2\) after repeated applications of \(g\). Each is an infinite set of discrete points contained in a tree fiber. The action on \(X \times Y\) is by isometries so the images of \(g^nv_1\) and \(g^nv_2\) should remain close for all \(n\). However, this is impossible \(\langle g\rangle v_1\) is contained in a vertical tree fiber while \(\langle g\rangle v_2\) is contained in a horizontal tree fiber.
    
    
    % By lemma \ref{lem:axeshausdorff} the axes in \(X \times Y\) are Hausdorff equivalent. Except, two unbounded sets from separate factors cannot be Hausdorff equivalent under the product metric, a contradiction.
...
Next we repeatedly apply lemma \ref{lem:fillingcocompact} and use the fact that filling preserves CCP  by construction to get that \(E:= Kxyz\) is cocompact and has CCP. By switching \ref{lem:switching} \(E\) has connected 1-dimensional fibers. 

\subsection{Topological properties of the Extended Core}
...
\begin{rmk}
    Our definition of VHD-Complex does not require that hyperplanes are injective on fundamental groups, only that hyperplanes are two-sided. The injectivity property is needed to obtain an honest graph of groups decomposition. For the VHD-Complex we consider the injectivity property follows from fact that there is a universal cover with simply-connected hyperplanes. In dimension two, for a VH-Complex the injectivty follows automatically from the two-sided requirement and the vertex link requirement.
\end{rmk}
        ...
        Lastly, \(E\subseteq T_1\times T_2\times T_3\) is a \(G\)-invariant subset and therefore the action of \(G\) on \(E\) is a covering space action.
