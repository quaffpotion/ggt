...
\begin{defn}
	[complexity]
    Given \(J=(T, \sigma_{1} \cdots \sigma_{N} , \{e_{1} , \ldots, e_{m}\})\) we define the {\em complexity} of \(J\) to be the sum of the number of times each edge is straddled. (We often take the set of edges to be the bad edges)
\end{defn}
\begin{defn}
	[path snipping]
...
\begin{lem}
    ...
    !!! seems we have two ways of proving this... one via induction and the other via checking multipaths ???
\end{lem}
