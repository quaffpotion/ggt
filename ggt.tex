...
\begin{lem}
	...
	\end{enumerate}
	\begin{proof}
		Let \(e\) be a bad outer geometric edge of \(T\) with respect to \(x_{0}\) and \(A\). Let \(i\) and \(j\) be the first and next integers such that \(\sigma_{i} , \sigma_{j}\) straddle \(e\). To get a valid snip, after possibly choosing an orientation of \(e\), take the largest inital (tail) segments of \(\sigma_{i}\) (\(\sigma_{j}\)) that don't include \(e\) (resp \(\overline{e}\) ). Perform a snip to create \(p'\) with subpaths that remain the same, become empty and are excluded, or in the case of \(\sigma_{i} , \sigma_{j}\) obtain the (possibly empty) initial segment (resp tail) \(\sigma'_{i}\) ( \(\sigma '_{j}\) ). 

		At this stage we must check that various things remain true. Consider the geodesic \(g\) from \(x_{ 0}\) to \(\partial e\). Let \(B\) be the union of the endpoints of \(\sigma_{k}\) for \(i\leq k\leq j\). \textcolor{blue}{It needs to be said, but it should be true that WLOG we can choose orientations so that for snips we start at \(e\) and end at \(\overline{e}\) and that \(e\) is chosen such that \(e^{-}\) is the halfspace containing \(x_{0}\). In particular, \(e^{-}\) will contain \(\partial e\).} We will consider four cases:
		\begin{enumerate}
