...
\begin{lem}
	If there exists \(K=(T,x_{0} ,\sigma_{1} \cdots \sigma_{N} , \{ e_{ 1} ,\ldots, e_{m}  \}, \lambda )\) satisfying properties below  with complexity \(c > 2\) then there exists \(K'\) also satisfying those properties with \(c' = c - 2\).
	\begin{enumerate}
		\item Every \(e_{i}\) is straddled
		\item 	 asdfasdf
		\item 	 asdaf
	\end{enumerate}
\end{lem}
