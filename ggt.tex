...
Our goal is to show that a well chosen cube complex which we call the core has a quotient that is a graph of spaces decomposition for \(G\) in three different ways. From this we will conclude that our group has dimension at least three which is impossible because we assumed it was VH.

    In terms of properties of the core we need three things: (1) hyperplanes are simply connected, (2) cocompact, (3) hierarchy of splittings gives enough dimension. We will take the edge spaces of our graph of spaces to be a subset of hyperplanes. 
...
To achieve this, we'll build \(C\) from a suitable \(G\)-invariant subcomplex \(K \subseteq \mathscr{T} \) by taking \(C = \sxyz\). This \(K\) will be the cellular hull of \(J\) which we define to be the image of a suitable map \(f\).

Ultimately, the injectivty on fundamental groups will follow from the core being one dimensional fiberwise connected and the second requirement that \(C\) be cocompact will follow from an argument that starts by bounding the size of fibers. Finally, having enough dimension will follow from some assumptions about our actions.

Our main result is the following:
\begin{cor*}
    Let \(G\) be the fundamental group of a compact VH-complex. Let \(T_{v}\), and \(T_{h}\) be the vertical and horizontal splittings. Considering {\em nice} actions up to their deformation spaces these are the only two such actions unless \(G\) satisfies {\em property A}.
\end{cor*}
...
\end{lem}
 ...
 At the end of Part 1 we have obtained our core. We still need to verify cocompactness. Indeed we have the following lemmas:
...
After this we have that the quotient of the core is a graph of groups decomposition for \(G\). With this splitting in hand we want to verify that it's made of successive graphs of groups of items of a certain dimension so we can apply Bieri.
