...
\begin{defn}
    ...
    \begin{enumerate}
        \item \(\sigma\) is a loop at \(x_{0} := \partial_{0} \sigma (1)\).
        \item \(\sigma_{1} (1) = \sigma_{N} (-1)\) 
        \item \(\lambda_{1} \neq \lambda_{N}\) 
        \item Only the first and last subpaths have endpoints at the basepoint.
...
\begin{defn}
    [Bad]
    An edge \(e\) in a tree \(T\) is {\em bad} with respect to \((\sigma = \sigma_{1} \cdots \sigma_{N} , \lambda , A)\) if \(e\) separates some \(a \in A\) from \(x_{0}\) and \(\lambda\) is constant on the set of subpaths that use \(e\).
\end{defn}
\begin{defn}
	[complexity]
...
\begin{lem}
    ...
    \end{proof}
\end{lem}
        Take \(\sigma\) as in the claim. Considering closed quadrants, there exists a sequence \(t_{1},\ldots,t_{n}\) such that \(\sigma (t_i)=:v_i\) are vertices in \(\{r_{1} \times \overline\delta \}\)  (here \(\delta\) is the distinguished vertical direction) with the property that \(\sigma (t_{1}) =p\), \(\sigma (t_{n}) = q\), and \(\sigma_{i} := \sigma \mid_{[t_{i} , t_{i + 1} ]}\) lie in quadrant \(i\) for \(0 < i < n\). Projecting, we obtain a sequence of subpaths in the closure of \(\delta\) in \(T_{2}\) that are colored based on their quadrant. Note that \(\overline \delta\) has a valence one vertex at \(r_{2}\) (because we took a direction) and subpaths don't end at \(p\) or \(q\) unless it's the first or last. This situation satisfies the conditions for our coloring lemma \ref{lem:coloring}.
        
        Multicolored paths in this vertical direction give paths contained in \(S_{x}\), we use the coloring lemma \ref{lem:coloring}  that shows there is a path between \(r\) and some other \(v_{i} \in S\).  This completes the proof.
    \end{proof}
\end{lem}
