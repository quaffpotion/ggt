...
For sake of a contradiction suppose $g\in G$ acts hyperbolically on $X$ but elliptically on $Y$ so it fixes some vertex $y\in Y$. Consider the stabilizer $G_y$ of $y\in Y$; it contains $g$. Now let $G_y$ act on $X$, the set $f^{-1}(y)$ is invariant because the map is a $G$-map and we are acting by automorphisms. This means the minimal subtree for the $G_y$ action on $X$ is contained in $f^{-1}(y)$.

Since $Y$ is not a single point, there exists some vertex $z\in Y$ with $y\neq z$. As before, $G_z$ stabilizes $f^{-1}(z)$ which is disjoint from $f^{-1}(y)$.

By local finiteness, $G_y$ and $G_z$ are commensurable. By \ref{pro:commintree} the minimal tree for $G_y$ is non-empty and equal to the minimal tree for $G_z$. Except now the connected minimal tree for $G_y$ is contained in two disjoint sets, a contradiction.
...
\end{lem}
\begin{proof}
        Suppose EX subet EY. By \fix{TODO proposition} this is equivalent to the existence of a G-map from X to Y. 
...
\begin{lem}
    ...
    \label{lem:minenough} 
    If the main theroem is true for minimal trees then it is true for trees in general
\end{lem}
