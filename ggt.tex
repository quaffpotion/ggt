...
\begin{claim*}
    ...
    \begin{proof}
        ...
        Take \(\sigma\) as in the claim. Considering closed quadrants, there exists a sequence \(t_{1},\ldots,t_{n}\) such that \(\sigma (t_i)=:v_i\) are vertices with the property that \(\sigma (t_{1}) =p\), \(\sigma (t_{n}) = q\), and \(\sigma_{i} := \sigma \mid_{[t_{i} , t_{i + 1} ]}\) lie in quadrant \(i\).
    \end{proof}
\end{claim*}
\end{document}
