...
\begin{lem}
    ...
    \begin{proof}
        ...
        Since \(p\) is in \(S_x \smallsetminus S\) there exist distinct points \(r\) and \(s\) in \(S\) that agree in all coordinates except the first where we have that \(p_{1} \in \text{cvxhull}_{T_{1}}  (\{r_{1} , s_{1}\})\). Now, because \(S\) is connected in all coordinate planes there is a path \(\sigma\) from \(r\) to \(s\) that lies in \(S \cap \pi_{2}^{-1} (p_{2})\). In fact, we can take \(\sigma\) to be a path that begins at \(r\) and ends at \(s\) but otherwise has \(T_{3}\) coordinates lying in exactly one closed direction of \(T_{3}\) at \(p_{3}\). We have factored out this situation into claim \ref{lem:verticalsubpath}.  
