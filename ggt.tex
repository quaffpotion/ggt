...
\begin{lem}
    ...
    \begin{proof}
        ...
        Take \(\sigma\) as in the claim \ref{lem:verticalsubpath}. Considering closed quadrants, there exists a sequence \(t_{1},\ldots,t_{n}\) such that \(\sigma (t_i)=:v_i\) are vertices in \(\{r_{1} \times \overline\delta \}\)  (here \(\delta\) is the distinguished vertical direction) with the property that \(\sigma (t_{1}) =p\), \(\sigma (t_{n}) = q\), and \(\sigma_{i} := \sigma \mid_{[t_{i} , t_{i + 1} ]}\) lie in quadrant \(i\) for \(0 < i < n\). Projecting, we obtain a sequence of subpaths in the closure of \(\delta\) in \(T_{2}\) that are colored based on their quadrant. Note that \(\overline \delta\) has a valence one vertex at \(r_{2}\) (because we took a direction) and subpaths don't end at \(p\) or \(q\) unless it's the first or last. This situation satisfies the conditions for our coloring lemma \ref{lem:coloring}.
