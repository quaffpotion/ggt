...
\subsection{Proof Walkthrough}
    Let \(X\) be a compact VH-complex and set \(G= \pi_{1} (X)\). Form \(\widetilde{K} \to K\) the universal cover. Note, \(G \curvearrowright \widetilde{K} \) freely and PDC.   
\subsection{General Plan}
Our goal is to show that a well chosen object which we call the core core is a graph of groups decomposition for \(G\), this follows from the corollary below which we will conclude from the theorem.
\begin{cor*}
...
To achieve this, we'll build \(C\) from a suitable \(G\)-invariant subcomplex \(S\) by taking \(C = \sxyz\). This \(S\) will be a cellular hull of \(\text{Im}(f)\) for a suitable \(f\).
