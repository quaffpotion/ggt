...
\end{lem}
\begin{rmk}
    For our purposes we only need that the map \(X_{12}^+\to T_3\) have connected fibers. From Guirardel, if \(G\) is finitely presented we can choose \(X_{12}^+\) to be simply connected. (??? Add some intuition about choosing \(K\) a suitable compact graph that carries all of the fundamental group)
\end{rmk}
