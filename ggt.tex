...
\begin{defn}
	[Finite Type] 
	An action of {\em finite type} is one where the tree is locally finite and vertex stabilizers are of type FP (i.e. of type \(FP_{n}\) for all \(n\) and of finite cohomological dimension; (clarification:) here \(FP_{n}\) is a condition on projective resolutions of \(\mathbb{Z}\) )
\end{defn}
