...
\begin{lem}
    ...
    \begin{proof}
        \textcolor{orange}{[warning] really we are using the fact that the image of this map contains more than two points. We could assume minimality and equivariance to get onto as well. Collapse maps are onto by construction}
        Suppose for sake of a contradiction that \(g\in G\) were hyperbolic for \(X\) and elliptic for \(Y\). Let \(y \in Y\) be some vertex fixed by the elliptic element \(g\) and \(G_y\) it's stabilizer. Since \(Y\) is not a single point, there is another vertex \(y\neq z \in Y\). Because \(Y\) is locally finite, \(G_y\) and \(G_z\) are commensurable. The pre-images of \(G\)-maps are invariant. By the construction of a collapse map, the preimage of vertices are connected. Putting these together we have that the preimages of vertices are invariant trees. This means that the minimal subtrees of \(G_y\) and \(G_z\) are contained in the disjoint preimages of \(y\) and \(z\) respectively. However, since they are commensurable and \(G_y\) contains the hyperbolic element \(g\), these minimal trees are non-empty and equal. This is a contradiction.
    \end{proof}
\end{lem}

\begin{thm}
    [Elliptic elements determine elliptic subgroups]
    \ref{thm:ellelesubgroups} 
...
\begin{thm}
    ...
    If \(X\) and \(Y\) are \(G\)-trees of finite type with finitely generated vertex and edge stabilizers then \(\mathcal{E}(X) \subseteq \mathcal{E}(Y) \Longrightarrow \mathcal{E}(X) = \mathcal{E}(Y) \). 
    \begin{proof}
        We first collapse our general \(G\)-map to a morphism. Then we use \ref{thm:folds} to factor the result as a sequence of folds. Then we use \ref{pro:foldfactor} and finite generation to get that same elliptic elements give us same elliptic subgroups which gives us same deformation space which means the second collapse was an elementary one which means we keep local finiteness.
    \end{proof}
\end{thm}
