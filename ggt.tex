...
\begin{claim*}
    ...
    \begin{proof}
        ...
        Take \(\sigma\) as in the claim. Considering closed quadrants, there exists a sequence \(t_{1},\ldots,t_{n}\) such that \(\sigma (t_i)=:v_i\) are vertices in \(\{r_{1} \times \overline\delta \}\)  (here \(\delta\) is the distinguished vertical direction) with the property that \(\sigma (t_{1}) =p\), \(\sigma (t_{n}) = q\), and \(\sigma_{i} := \sigma \mid_{[t_{i} , t_{i + 1} ]}\) lie in quadrant \(i\). Projecting, we obtain a sequence of subpaths in the closure of \(\delta\) in \(T_{2}\) that are colored based on their quadrant. Multicolored paths in this vertical direction give paths contained in \(S_{x}\), we use the combinatorial lemma REFERENCE that shows there is a path between \(r\) and some other \(v_{i} \in S\).  
    \end{proof}
\end{claim*}
\end{document}
